    ...
    class Program
    {
        ...
        */
        ...
        private static string RemoveVowels(string s)
        {
            try
            {
                ...
                {
                    if (!n.Contains(s[i]))  // Checking if vowels are present in given string
                        final_string += s[i];   // concatenating remaining characters.
                }
                return final_string;
       ...
       */
        ...
        private static bool ArrayStringsAreEqual(string[] bulls_string1, string[] bulls_string2)
        {
            ...
            {
                // write your code here.
                if (string.Join("", bulls_string1) == string.Join("", bulls_string2))   //Joining and comparing Arrays.
                {
                    return true;
                }
                else
        ...
        private static int SumOfUnique(int[] bull_bucks)
        {
            try
            {
                ...
                foreach (int a in bull_bucks)
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        if (a == bull_bucks[i]) // checking for unique elements
                        {
                            count = count + 1;
                        }
                    }
                    if (count == 1) // if condition satisfies it is unique
                    {
                        s = s + a;
                    }
                }
        ...
        */
        ...
        private static int DiagonalSum(int[,] bulls_grid)
        {
            try
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    s = s + bulls_grid[j, j] + bulls_grid[j, n - j - 1]; // calculation of diagnol elements sum
                }
                if (n % 2 != 0)
                {
                    int z = (n - 1) / 2;
                    return s - bulls_grid[z, z];
                }
                else
                {
                    return s;
        ...
        */
        ...
        private static string RestoreString(string bulls_string, int[] indices)
        {
            try
            {
                ...
                {
                    int index = indices[i];
                    a[index] = Convert.ToString(bulls_string[i]);   //getting the string according to given index
                }
                for (int i = 0; i < bulls_string.Length; i++)
                {
                    final_string = final_string + a[i];
                }
                return final_string;
        ...
        private static string ReversePrefix(string bulls_string6, char ch)
        {
            try
            {
                ...
                for (int i = 0; i < bulls_string6.Length; i++)
                {
                    ...
                    {
                        x = bulls_string6.Split(ch);    // splitting the string at the given character
                    }
                }
                string r = "";
                for (int j = x[0].Length - 1; j >= 0; j--)  // reversing the string
                {
                    r = r + bulls_string6[j];
                }
                String prefix_string = ch + r + x[1];
